...
# MAGIC %scala
# MAGIC # If you have the File > Upload Data menu option, follow the instructions in the previous cell to upload the data from your local machine.
# MAGIC # The generated code, including the required edits described in the previous cell, is shown here for reference.
# MAGIC
# MAGIC # In the following lines, replace <username> with your username.
# MAGIC # white_wine = pd.read_csv("/dbfs/FileStore/shared_uploads/<username>/winequality_white.csv", sep=';')
# MAGIC # red_wine = pd.read_csv("/dbfs/FileStore/shared_uploads/<username>/winequality_red.csv", sep=';')
# MAGIC
# MAGIC # If you do not have the File > Upload Data menu option, uncomment and run these lines to load the dataset.
# MAGIC

# COMMAND ----------

# MAGIC %scala
# MAGIC val storage_account_name = "stwinewesteurope"
# MAGIC val storage_account_access_key = "llsOcqUZ1apOSzLrD6jz7hzjH1CJIiSx2AUd/NlQfTIcQrhabXWRQfgjBGlrjHFWtzrT2L4mHbsM+ASt1tMByg=="
# MAGIC
...
import pandas as pd
file_location_1 = "wasbs://data@stwinewesteurope.blob.core.windows.net/m08sparkml/winequality/winequality-red.csv"
file_location_2 = "wasbs://data@stwinewesteurope.blob.core.windows.net/m08sparkml/winequality/winequality-white.csv"
file_type = "csv"

df1 = spark.read.csv(file_location_1, sep=";", header=True)
df2 = spark.read.csv(file_location_2, sep=";", header=True)

red_wine = df1.toPandas()
white_wine = df2.toPandas()
red_wine.sample(5)
